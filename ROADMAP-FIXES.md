...
### F4.c-5 ✅ Cobro automatico recurrente — CLOSED
- **Closed**: 2026-08-05. La fase del dinero: por primera vez el sistema gasta plata de un usuario sin que nadie mire.
- **Arquitectura (decidida por el spike F4.c-2, no rediseñada)**: el cron SOLO INICIA; el webhook —ya probado en produccion— otorga el plan. El cron NUNCA escribe plan, plan_expira ni fue_pago, y NUNCA interpreta la respuesta de la API como autorizacion.
- **Referencia DETERMINISTICA por periodo de facturacion** en lugar de Date.now(): una corrida duplicada produce la MISMA referencia y el 422 de Wompi bloquea el segundo cobro. Segunda red ademas de facturas.numero.
- **La cancelacion SIEMPRE gana, por partida doble**: el barrido de cobro corre DESPUES del de cambios programados, Y debeCobrarse se niega si hay plan_programado. El orden que antes prevenia esto existia POR SUERTE (su comentario justificaba deduplicacion de emails, no orden de cobros); ahora la regla esta fijada en los DOS sitios. VERIFICADO: con cancelacion programada, cobrosIniciados=0.
- **Reclamo ANTES de cobrar, asimetria deliberada**: se marca ultimo_cobro_periodo y despues se llama a Wompi. Morir entre reclamo y POST pierde UNA renovacion (recuperable con un email); morir entre POST y escritura cobraria DOS VECES manana (irrecuperable). Se elige el error barato, comentado con el POR QUE.
- **El reclamo NO se libera al fallar**: un rechazo no prueba que la plata no se movio (existen timeouts), asi que liberarlo permitiria doble cobro. Misma asimetria, aplicada consistentemente.
- **Se cobra 3 DIAS ANTES del vencimiento**, no el dia: un fallo deja el plan vivo y da margen para pago manual. La UI anuncia AMBAS fechas porque anunciar plan_expira como "proximo cobro" seria mentir: la plata se mueve antes.
- **BUG encontrado en smoke con dinero moviendose**: una transaccion DECLINED escribio una fila en facturas con estado "rechazada". La regla —los fallos JAMAS tocan facturas— estaba implementada en el cron pero el webhook no la respetaba, y como el webhook solo inserta si no existe factura, esa fila bloqueaba PARA SIEMPRE la factura real de ese periodo. Corregido: los no-APPROVED registran en fuentes_pago y disparan el email de cobro fallido, nunca escriben factura. La fila huerfana se borro a mano.
- **Copy por fila, nunca global**: solo un usuario enrolado CON cobro_automatico ve "Se renueva el <fecha>"; el resto conserva "Tu plan vence el <fecha>. No se renueva automaticamente." Un flag global seria mentirle a la mitad.
- **Opt-in explicito**: cobro_automatico arranca en false para TODOS (verificado: con_cobro=0 antes de desplegar). El disclosure declara monto, fecha real del cobro, metodo enmascarado, que se repite hasta cancelar, y COMO APAGARLO antes de encenderlo.
- **Politica de fallos v1, deliberadamente simple**: sin escalera de reintentos. Se registra en fuentes_pago, se envia email con enlace de pago manual, y el barrido de vencimiento sigue su curso.
- **Techo de Vercel Hobby**: ~7 cobros por corrida (limite de 10s). Vercel Pro se vuelve obligatorio alrededor de 70-100 suscriptores; medible con cobrosIniciados en el log.
- **Verificado en produccion**: cobro disparado (201) con reclamo de periodo; segunda corrida inmediata cobrosIniciados=0 (idempotencia); con cancelacion programada cobrosIniciados=0; tras el fix, un cobro rechazado NO genero factura y el email de cobro fallido llego.
- **NO verificable en sandbox**: el sandbox de Wompi RECHAZA los cobros automaticos contra fuentes Nequi (webhook ERROR). No sabemos si es limitacion del ambiente o comportamiento real; solo un cobro en produccion lo dira. El camino de exito (webhook APPROVED -> plan otorgado + factura + email de renovacion) quedo SIN ejercitar de punta a punta.
- **Commits**: a2b6460 (implementacion), c06e43d (fix de facturas).

### F4.c-4 ✅ Enrolamiento Nequi: guardar un metodo de pago (sin cobrar) — CLOSED
- **Closed**: 2026-07-30. Primera fase de F4.c visible para el usuario. GUARDA una fuente de pago Nequi; NO cobra nada. Un enrolamiento abandonado deja una fila PENDING y nada mas: sin cargo, sin cambio de plan. Por eso era la fase segura para ir primero.
- **Arquitectura**: capa nueva src/lib/wompiApi.ts (server-only, unica que toca la red y las llaves). Ruta de enrolamiento: par de aceptacion FRESCO -> POST /v1/tokens/nequi -> fila PENDING con el token id y el telefono ENMASCARADO. Rama nueva del webhook para nequi_token.updated, colocada ANTES del guard de data.transaction: ese evento llega con raiz data.nequi_token y hasta hoy se descartaba en silencio pareciendo exito. La verificacion de firma no necesito cambios gracias a la resolucion dinamica de signature.properties — se valida la decision de F4.a-2 de no cablear los campos.
- **DOS pares de acceptance tokens, a proposito**: el del enrolamiento es EVIDENCIA (lo que el usuario tenia a la vista al aceptar) y nunca se consume; el del webhook es el que Wompi consume al crear la fuente. Reutilizar el primero daria 422 "ya fue usado" — son de un solo uso (F4.c-2).
- **Idempotencia por compare-and-swap**: el update lleva .eq('estado','PENDING'), asi que si dos entregas del mismo evento corren a la vez solo UNA escribe; la que pierde devuelve 200 idempotente. El 23505 del indice unico tambien se atrapa y devuelve 200, nunca 500.
- **Quitar el metodo es una operacion NUESTRA**: no existe endpoint en Wompi para anular una fuente normal (F4.c-2), asi que revocar escribe VOIDED + revocada_en + motivo Y una fila REVOCADO en consentimientos_pago. Esa fila es la UNICA evidencia exhibible de que dejamos de cobrar; Wompi conserva la fuente para siempre.
- **Correcciones al esquema forzadas por la implementacion** (no se veian disenando en abstracto): wompi_payment_source_id paso a NULLABLE (la fuente no existe mientras el enrolamiento esta PENDING) y se agrego wompi_token_id + indice unico (el evento nequi_token.updated NO trae reference ni restaurante: ese token es el UNICO enlace con nuestra fila). fuentes_pago paso de 18 a 19 columnas.
- **BUG encontrado en smoke — leccion permanente**: POST /v1/tokens/nequi devolvia 401 INVALID_ACCESS_TOKEN porque se le mandaba la llave PRIVADA. LA REGLA: tokenizar es PUBLICO (es la operacion pensada para ser segura desde el cliente); guardar una fuente y cobrar son PRIVADOS. Esta escrita al tope de wompiApi.ts, lo que ademas evita que F4.c-6 herede el mismo bug en /v1/tokens/cards. Diagnostico agravado porque AMBAS llaves miden 41 caracteres, asi que el log de longitudes parecia una pista y no lo era: ahora cada llamada declara y loguea QUE llave uso (nunca el valor), asi un 401 nombra su propia causa en una linea.
- **Verificado en produccion (ciclo completo)**: enrolamiento -> PENDING -> evento de Wompi -> POST /payment_sources 201 -> AVAILABLE con payment_source_id 357841; telefono guardado como ***4564 (el numero completo NO aparece en ninguna fila); consentimiento OTORGADO con contract_id 472/439 y los dos permalinks reales, CERO JWT; plan, plan_expira, fue_pago y cobro_automatico=false SIN TOCAR (guardar un metodo no es pagar); revocacion -> VOIDED + REVOCADO con motivo; y re-enrolamiento posterior EXITOSO (una fuente revocada no bloquea el indice unico).
- **NO probable contra Wompi**: el sandbox AUTO-APRUEBA los tokens Nequi, asi que las ramas "el usuario nunca aprobo" y "el usuario rechazo" no se pueden ejercitar ahi. La UI del estado PENDING es correcta por construccion; esas ramas necesitan mocks propios.
- **Commits**: 79c6b84 (implementacion), 8013747 (fix de llaves).
...
- **Implementacion**: vercel.json con cron diario "0 5 * * *" UTC = medianoche COT; GET /api/cron/suscripciones con guard de CRON_SECRET timing-safe (mismo patron que el webhook: .trim(), longitud logueada nunca el valor, 500 si falta, 401 generico si no coincide), service role. DOS barridos de reconciliacion: (a) aplicar cambios programados donde fecha_cambio_programado <= hoy; (b) expirar planes pagos donde plan_expira < hoy (estricto: el dia de vencimiento el usuario conserva el dia). Ambos usan <= / < y NUNCA = hoy, asi una corrida perdida o duplicada se auto-sana (Vercel no reintenta crons y la entrega es best-effort).
- **fue_pago JAMAS se toca**: resetearlo republicaria fotos ocultas por fotosGate — fuga de datos, no solo un bug de facturacion. Verificado en produccion: tras expirar, fue_pago=true intacto.
- **Fechas COT**: comparacion de strings 'YYYY-MM-DD' via fechaColombia (lexicografico == cronologico, sin Date de por medio). Primer uso SERVER-side del formateador Intl del repo, con asercion de forma que aborta ANTES de escribir si el reloj degrada. VERIFICADO en el peor escenario: corrida a las 21:47 COT (cuando UTC ya esta en el dia siguiente) reporto fecha=2026-07-28 correcta. Cierra la familia BL.29/BL.41 del lado del cron.
- **Emails**: dos plantillas nuevas (plan vencido, cambio programado aplicado) via el seam sin cookies lib/email/notificaciones.ts; destinatario resuelto SIEMPRE de restaurantes.usuario_id -> auth.admin.getUserById, nunca de un payload. Fallo de email jamas revierte la escritura ni aborta el barrido.
- **Resiliencia**: una fila que falla no aborta el barrido; 200 con conteos por barrido (sin valores personales); 500 reservado a "no pudo arrancar".
- **Smoke en produccion**: expirados=1 con plan pago vencido -> fila en gratis, plan_expira conservado como historico, fue_pago intacto, email recibido; segunda corrida inmediata devolvio cero (idempotencia probada).
- **Incluye tambien**: el copy honesto de renovacion que quedo sin commitear la ronda anterior ("Tu plan vence el <fecha>" + "No se renueva automaticamente"), porque /suscripcion prometia una renovacion automatica inexistente.
- **Config manual**: CRON_SECRET en Vercel (Sensitive, Production). Las variables se congelan en el build: cargar la variable NO basta, hay que redeployar (se perdieron dos corridas por esto).
- **Commit**: b616b39.

### F4.b-1 ✅ Cancelacion diferida + renovacion que suma en vez de resetear — CLOSED
- **Closed**: 2026-07-28. DOS bugs vivos en produccion, ambos costandole dinero al usuario.
- **BUG A — cancelar borraba el tiempo pagado al instante**: quien pagaba Pro anual el lunes y cancelaba el martes perdia todo. Estandar de la industria (Netflix, Spotify, Adobe): cancelar = NO renovar, el acceso corre hasta el fin del periodo pagado. Ahora cancelar con tiempo restante AGENDA el cambio (columnas nuevas plan_programado + fecha_cambio_programado, migracion manual verificada pieza por pieza) dejando plan/periodo_plan/fue_pago intactos; /suscripcion muestra el estado de retencion con accion de Reactivar (patron "Restart membership"). Sin tiempo restante se mantiene la bajada inmediata. Fix colado: cancelar ya no sobreescribe periodo_plan con el valor del toggle de la UI.
- **BUG B — pagar temprano quemaba los dias restantes**: calcularPlanExpira calculaba desde HOY, asi que quien tenia 20 dias y renovaba los perdia. Ahora extiende desde el vencimiento existente SOLO cuando el plan Y el periodo comprados coinciden con la fila (renovacion); upgrade o cambio de periodo siguen arrancando ciclo nuevo desde hoy, preservando la decision registrada de sin-prorrateo. El webhook limpia cualquier cambio agendado al recibir un pago: quien paga no se esta bajando.
- **Nota**: el helper valida rango de mes/dia, no solo forma — '2026-13-01' pasaba el regex y rodaba a 2027 (bug encontrado por los tests del propio CC).
- **Commit**: 82732d1.

### BL.46 🟡 Referidos: promesa comercial sin mecanica detras — DESENLAZADO 2026-07-27, ABIERTO
- **Found**: 2026-07-27 (investigacion read-only pedida por Julian antes de tocar nada). /referidos promete "Invita a un restaurante. Cuando pague su primer mes, ambos reciben 1 mes gratis" y el paso 3 del explicador dice "automaticamente". Nada de eso ocurre: quien invitaba a un colega no recibia nada y ademas veia la lista de invitados siempre vacia. Era una promesa comercial rota en produccion.
- **Lo que SI existe**: la pagina (src/app/referidos/page.tsx) genera el codigo una vez si falta (:28-38, 4 letras del nombre + 3 chars aleatorios base36 -> restaurantes.codigo_referido), arma el enlace con urlRegistroRef (lib/urls.ts:13 -> /registro?ref=CODIGO), comparte por clipboard/WhatsApp/navigator.share (:56-77) y LEE la tabla via useReferidos (hooks/data/useReferidos.ts:14-24, filtro referidor_id + join al nombre del invitado). La UI de resumen e historial esta completa.
- **Lo que NO existe**: (a) /registro NUNCA lee el ?ref= — solo mira 'google' y 'paso' (app/registro/page.tsx:15-16), asi que el codigo del enlace se descarta en el registro; la tarjeta "Tu codigo" (:137) ademas dice que el invitado puede escribirlo al registrarse y ese campo no existe; (b) CERO escritores de la tabla referidos en todo src — solo un select (useReferidos:18) y el delete de borrado de cuenta (config/page.tsx:327), por eso la lista sale siempre vacia y 'estado' nunca cambia; (c) NINGUN codigo otorga un mes gratis a nadie — el unico escritor de plan_expira es el webhook de Wompi (api/wompi/eventos/route.ts:104) y calcula desde la fecha de hoy, sin mirar referidos. Los campos de recompensa que existen son cascaron: beneficio_aplicado se mapea (referidos:44) y no se pinta nunca, y Suscripcion.credito_referidos (types:297) pertenece a una tabla 'suscripciones' que ningun codigo consulta.
- **No verificable desde el repo**: no hay migraciones ni SQL versionado, asi que un trigger o RPC del lado de Postgres no se puede descartar leyendo codigo (ningun .rpc() se llama). Todo lo visible dice que la recompensa nunca se construyo.
- **Por que se desenlazo (2026-07-27)**: se quito la promesa del producto sin borrar el trabajo. La ruta /referidos sigue viva y funcional, solo sin enlaces. Dos call sites comentados, a descomentar cuando esto se implemente: (1) fila "Invitar restaurantes / Gana meses gratis" del dropdown de perfil en app/dashboard/page.tsx (el import de UserPlus y la rama de color verde se dejaron inertes a proposito para que restaurar sea un solo descomentar); (2) tarjeta verde "Credito referidos" al final de app/suscripcion/page.tsx. NO se toco nada de la capa de datos: tabla, useReferidos, generacion de codigo_referido y limpieza en borrado de cuenta siguen igual.
- **Hook point cuando se retome**: la rama APPROVED del webhook de Wompi (api/wompi/eventos/route.ts), justo despues del update de plan (:100-107) y junto al insert de factura (:109-123). Ahi ya estan alineadas las cuatro piezas dificiles: service role (createAdminClient, :70 — obligatorio porque el referidor es OTRO tenant que la sesion del pagador jamas podria escribir bajo RLS), identidad confiable (restauranteId sale de la referencia firmada, :64-68), "primera vez que paga" ya calculable (restPrev se lee ANTES del update en :92-97, y fue_pago===false en ese instante ES el latch one-way de STRATEGIC.2), e idempotencia ya resuelta (facturas.numero == reference, :73-77 y :87-89, evita doble otorgamiento en reintentos de Wompi). Seam secundario: notificarPlanActivo (:141) para avisar al referidor, con la misma regla de identidad (dueno via auth.users, nunca el email del pagador).
- **El eslabon que falta**: nada enlaza al restaurante que paga con quien lo refirio. Primero el REGISTRO debe persistir el ?ref= (fila en referidos o columna en restaurantes); sin eso el webhook no tiene a quien premiar. Ojo tambien: calcularPlanExpira (lib/wompi.ts:84-85) calcula desde HOY, asi que el mes de premio debe SUMARSE al plan_expira existente del referidor, nunca recalcularse (recalcular le acortaria la suscripcion vigente).
- **Riesgo de unicidad del codigo (resolver ANTES de mover dinero)**: el codigo son 4 letras del nombre + 3 chars de Math.random en base36 (referidos:33), sin constraint unique visible en codigo. Dos restaurantes de nombre parecido pueden colisionar y la recompensa se atribuiria a la cuenta equivocada. Mientras el codigo solo pinta una tarjeta es cosmetico; en el momento en que decide plata, es un bug de dinero.
- **5 decisiones de producto abiertas (nadie las ha tomado)**: (1) auto-referido — hoy nada lo impide si se cablea; (2) tope por referidor — la UI implica ilimitado ("Meses ganados" es el conteo crudo de activos, :176), o sea N invitados = N meses gratis, pasivo sin techo; (3) referidor en plan gratis cuando cae el premio — plan_expira no significa nada para gratis (/suscripcion muestra "Sin fecha de renovacion"), asi que hay que decidir entre acumular credito, regalar un mes de plan pago, o no hacer nada; (4) reverso por devolucion o churn — el webhook registra VOIDED/DECLINED/ERROR como facturas (:157-173) pero NUNCA revierte plan ni plan_expira, y un invitado puede pagar un mes y bajarse a gratis el mismo dia (suscripcion bajarAGratis), premio ya otorgado y sin ruta de clawback; (5) que significa 'activo' vs 'pendiente' — la maquina de estados no tiene escritor, asi que hoy no significa nada, pero la UI ya paga visualmente con "+1 mes gratis" en activo (:213-217).
- **Trampa de tipos**: la interface Referido (types/index.ts:254-262) describe columnas que NO son las reales — dice restaurante_origen_id / restaurante_invitado_id / nombre_invitado / mes_gratis_aplicado, mientras el codigo consulta referidor_id / referido_id / beneficio_aplicado. Tiene CERO imports (useReferidos declara su propio ReferidoRow). Misma clase de trampa que el viejo tipo Factura, cuyos nombres legacy causaban 400 al consultar. Quien retome esto y confie en Referido escribira queries que fallan.
- **Acceptance criteria**: el registro captura y persiste el ?ref=; el webhook otorga el premio en la primera compra real del invitado sumando al plan_expira del referidor, idempotente y con las 5 decisiones de arriba resueltas; la lista de /referidos se puebla de verdad; y solo entonces se descomentan los dos call sites.
- **Priority**: 🟡 (no hay bug activo mientras este desenlazado — la urgencia era la promesa visible, ya retirada; retomar cuando el crecimiento por referidos sea prioridad de negocio).
